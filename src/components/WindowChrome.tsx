 ...
 *   at x=10px from the window. We park the chrome at left: 14px
 *   so the traffic lights sit 4px inside the panel's left
 *   edge — close enough to read as "inside the panel" when the
 *   panel is open, but not glued to the absolute window edge.
 *   The chrome's top: 14px mirrors the side offset and gives
 *   the same 4px inset from the window's top edge.
 */
...
export function WindowChrome() {
  return (
    <div
      ...
      className="absolute top-[14px] left-[14px] z-30 h-7 flex items-center gap-1.5 select-none"
    >
      <TrafficLights />
      <PanelToggle />
    </div>
